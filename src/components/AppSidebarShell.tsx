...
import {
  ...
  TooltipProvider,
  TooltipTrigger,
  TooltipContent as TooltipContentComp,
} from '@blinkdotnew/ui'
import {
  LayoutDashboard,
  PlusCircle,
...
const NAV_ITEMS: NavItemDef[] = [
  ...
  { to: '/create', icon: <PlusCircle className="h-4 w-4" />, label: 'Create Receipt' },
  { to: '/history', icon: <History className="h-4 w-4" />, label: 'Receipt History' },
  { to: '/banks', icon: <Building2 className="h-4 w-4" />, label: 'Banks' },
      ...
      >
        ...
        >
          {!collapsed && (
            <>
              ...
              <span className="flex-1 font-semibold text-sm truncate">Receipt Maker</span>
            </>
          )}
          <Tooltip>
        ...
        >
          ...
          {collapsed ? (
              ...
              <TooltipTrigger asChild>
                <button className="flex items-center justify-center h-8 w-8 rounded-md hover:bg-accent transition-colors cursor-pointer">
                  <Avatar className="h-6 w-6 shrink-0">
                    <AvatarFallback className="text-[10px] bg-muted">{user?.email?.[0].toUpperCase() || 'U'}</AvatarFallback>
                  </Avatar>
                </button>
              </TooltipTrigger>
            ...
            <div className="flex items-center gap-2 rounded-md hover:bg-accent transition-colors cursor-pointer w-full px-2 py-1.5">
              <Avatar className="h-6 w-6 shrink-0">
                <AvatarFallback className="text-[10px] bg-muted">{user?.email?.[0].toUpperCase() || 'U'}</AvatarFallback>
              </Avatar>
